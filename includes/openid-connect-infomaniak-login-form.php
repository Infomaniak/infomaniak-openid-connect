 ...
 * @license   http://www.gnu.org/licenses/gpl-2.0.txt GPL-2.0+
 */
class OpenID_Connect_Infomaniak_Login_Form {
	 ...
	 * @param OpenID_Connect_Infomaniak_Client_Wrapper  $client_wrapper A plugin client wrapper object instance.
	 */
	...
	/**
	 * Create an instance of the OpenID_Connect_Infomaniak_Login_Form class.
	 *
	 * @param OpenID_Connect_Infomaniak_Option_Settings $settings       A plugin settings object instance.
	 * @param OpenID_Connect_Infomaniak_Client_Wrapper  $client_wrapper A plugin client wrapper object instance.
	 *
	...
	public static function register( $settings, $client_wrapper ) {
		...
		add_shortcode( 'infomaniak_connect_generic_login_button', array( $login_form, 'make_login_button' ) );
	...
	public function make_error_output( $error_code, $error_message ) {
		...
		<div id="login_error"><?php // translators: %1$s is the error code from the IDP. ?>
			<strong><?php printf( esc_html__( 'ERROR (%1$s)', 'infomaniak-openid-connect-infomaniak' ), esc_html( $error_code ) ); ?>: </strong>
	...
	public function make_login_button( $atts = array() ) {
		...
		$atts = shortcode_atts(
			array(
				'button_text' => __( 'Login with Infomaniak OpenID Connect', 'infomaniak-openid-connect-infomaniak' ),
			),
			$atts,
		...
		$login_button = <<<HTML
<div class="infomaniak-connect-login-button" style="margin: 1em 0; text-align: center;">
